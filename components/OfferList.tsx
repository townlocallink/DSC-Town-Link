...
import { Offer, Order, UserProfile, ProductRequest } from '../types';
...
const OfferList: React.FC<OfferListProps> = ({ offers, requests, user, onAccept, onUpdateUser, onOpenChat }) => {
  ...
  const handleConfirmOrder = () => {
    ...
    // Fix: Using 'pending_assignment' instead of 'pending' to satisfy the Order interface status type.
    const newOrder: Order = {
      ...
      shopName: addressModalOffer.shopName,
      amountToCollect: addressModalOffer.price,
  ...
  return (
    <div className="space-y-4">
      {offers.map(offer => {
        const isNew = (Date.now() - offer.createdAt) < (5 * 60 * 1000); // 5 minutes
        const relatedRequest = requests.find(r => r.id === offer.requestId);
        
        return (
          <div key={offer.id} className={`bg-white p-6 rounded-[32px] shadow-sm border transition relative ${isNew ? 'border-indigo-500 ring-2 ring-indigo-50' : 'border-gray-100 hover:border-indigo-600'}`}>
            {isNew && (
              <span className="absolute -top-2 left-6 bg-indigo-600 text-white text-[8px] font-black uppercase px-2 py-0.5 rounded-full animate-bounce">
                New Quote!
              </span>
            )}
            ...
            {/* Request Link UI */}
            {relatedRequest && (
              <div className="mb-4 bg-gray-50 p-3 rounded-2xl border border-gray-100 flex items-start gap-2">
                <span className="text-[10px] mt-0.5 opacity-50">📦</span>
                <div>
                   <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest">Regarding your request:</p>
                   <p className="text-[10px] font-bold text-gray-600 line-clamp-1 italic">"{relatedRequest.description}"</p>
                </div>
              </div>
            )}
