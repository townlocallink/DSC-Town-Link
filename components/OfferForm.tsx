...
const OfferForm: React.FC<OfferFormProps> = ({ request, shop, onClose, onSubmit }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (isSubmitting || !price) return;
    
    setIsSubmitting(true);
    
    // Construct offer object cleanly to avoid undefined fields
    ...
    if (message.trim()) newOffer.message = message.trim();
    if (image) newOffer.productImage = image;
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <button 
        ...
        className="w-full bg-indigo-600 text-white font-black p-5 rounded-2xl mt-2 hover:bg-indigo-700 transition shadow-xl shadow-indigo-100 uppercase tracking-widest text-[10px] disabled:opacity-50"
      >
